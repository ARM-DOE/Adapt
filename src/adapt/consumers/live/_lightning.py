...
"""Join LMA lightning stats (lma_cell_stats) onto a track's scan timeline.
...
def merge_lightning(track_df: pd.DataFrame, lma_df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    if lma_df is None or lma_df.empty or "time_bin" not in lma_df.columns:
        return track_df
    ...
    lma = lma_df.copy()
    lma["_bin"] = pd.to_datetime(lma["time_bin"], utc=True)
