...
"""Deterministic teardown for the process-global store-registry cache.

``StoreRegistry.get_instance`` caches one WAL connection per store root for the
lifetime of the process. Without a release path that cache accumulates open file
descriptors (db + ``-wal`` + ``-shm`` per root) and exhausts a low
...
def _cache_twenty_registries(tmp_path) -> list:
    """Open one live registry connection per store root and return the roots."""
    roots = []
    for i in range(20):
        root = init_store(tmp_path / f"store{i}")
        StoreRegistry.get_instance(root).list_collections()  # first query opens the WAL connection
        roots.append(root)
    return roots
...
def test_close_all_returns_the_descriptor_count_to_baseline(tmp_path):
    ...
    StoreRegistry.close_all()  # start from a clean cache, independent of other tests
    gc.collect()
...
def test_close_all_leaves_every_store_deletable(tmp_path):
    """After close_all, nothing may still be holding a store's files.
    ...
    for users, whose store directories become unremovable.
    """
    StoreRegistry.close_all()
...
def test_get_instance_after_close_all_returns_a_working_registry(tmp_path):
    ...
    StoreRegistry.get_instance(root)
    StoreRegistry.close_all()

    registry = StoreRegistry.get_instance(root)
    registry.register_collection("KTST", source_kind="nexrad")  # a real write proves it is live
    assert [c["collection_id"] for c in registry.list_collections()] == ["KTST"]
