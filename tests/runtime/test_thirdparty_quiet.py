...
Py-ART prints a citation block on import; it is pulled in transitively by
``nexradaws`` (acquisition -> sources -> runtime), which imports pyart at its own
import time — *before* any Adapt module could set PYART_QUIET. So the suppression
must live in the package root (adapt/__init__), which runs before every submodule.
Verified in clean subprocesses importing the runtime path that triggers nexradaws.
"""
...
def test_importing_runtime_does_not_print_pyart_citation():
    # adapt.runtime pulls nexradaws -> pyart; the banner must still be suppressed.
